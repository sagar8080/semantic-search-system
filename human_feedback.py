...
import copy # To make deep copies for editing state safely
...
@st.cache_data # Cache initial loading based on file content hash
def load_initial_data(uploaded_file):
    """Loads data from the uploaded JSON file."""
...
# --- CRUD & Feedback Functions (Modify st.session_state) ---
...
def rename_topic_state(old_topic_name, new_topic_name):
    if old_topic_name and new_topic_name and old_topic_name in st.session_state[SESSION_STATE_KEY]:
        if new_topic_name in st.session_state[SESSION_STATE_KEY] and old_topic_name != new_topic_name:
            ...
            # Preserve order if it matters, otherwise dict reassignment is fine
            new_data = {}
            ...
            # Update mismatches if the topic name changed
            ...
            st.rerun() # Rerun immediately to update selectboxes
    else:
...
def save_data_to_json(topic_data, filename="updated_topic_data.json"):
    try:
        # Ensure data integrity before saving (e.g., sort phrases within topics)
...
if st.session_state[SESSION_STATE_KEY] is not None:
    ...
    # Save Button
    if st.sidebar.button("💾 Save Changes to JSON", key="save_btn"):
        save_data_to_json(current_data)
